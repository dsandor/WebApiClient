...
namespace SimpleWebApi.WindowsPhone8
{
    /// <summary>
    ...
    class WebApiClient
    {
        ...
        public static async Task<TReturnType> Post<TReturnType>(string apiUrl, string authToken, string data)
        {
        ...
        /// <returns>A strongly typed object that was returned from te WebApi call.</returns>
        public static async Task<TReturnType> Post<TReturnType, TRequestType>(string apiUrl, string authToken, TRequestType request)
        {
            string data = await JsonConvert.SerializeObjectAsync(request);
            return await Post<TReturnType>(apiUrl, authToken, data);
        }
        ...
        /// <param name="data">The data to post.</param>
        /// <returns>A strongly typed object that was returned from te WebApi call.</returns>
        public static async Task<TReturnType> Post<TReturnType>(string apiUrl, Dictionary<string,string> headers, string data)
        {
            var webClient = new WebClient();
            var uri       = new Uri(apiUrl);
        ...
        /// <returns>A strongly typed object returned from the WebApi call.</returns>
        public static async Task<TReturnType> Get<TReturnType>(string apiUrl, string authToken)
        {
            var webClient = new WebClient();
            ...
            return await Get<TReturnType>(webClient, uri);
        }
        ...
        /// <returns>A strongly typed object returned from the WebApi call.</returns>
        public static async Task<TReturnType> Get<TReturnType>(string apiUrl, Dictionary<string, string> headers)
        {
            var webClient = new WebClient();
            var uri       = new Uri(apiUrl);
